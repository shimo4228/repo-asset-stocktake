...
description: Audit a project repository's non-code assets — tool configs, CI/GitHub workflows, runbooks and other docs — for diminished value, and assign Keep/Update/Retire/Merge verdicts. Every non-code asset serves a consumer (a tool invocation, a CI trigger, a human reader) — this skill flags assets whose consumer has vanished or which no longer serve it. Use when the user says "audit my repo assets", "which configs/workflows/runbooks are dead", "repo asset stocktake", "非コード資産を棚卸しして", "使われてない設定/workflow/runbook を洗い出して", or "/repo-asset-stocktake". NOT for dead CODE or code-loaded data files (that is refactor-clean, a structural sweep), NOT for documentation-role overlap across CLAUDE.md/CODEMAPS/ADR/README (that is context-sync), NOT for ~/.claude config GC across hooks/permissions/MCP (that is config-gc), and NOT for auditing skills or rules (that is skill-stocktake / rules-stocktake).
